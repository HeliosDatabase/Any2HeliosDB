...
asciinema rec --overwrite -q --cols 100 --rows 32 -i 2 \
  -t "Any2HeliosDB - migrate Oracle HR -> HeliosDB-Nano" \
  -c "bash $DIR/demo-migrate-clean.sh" \
  "$DIR/oracle-nano-migrate.cast"
echo "wrote: $DIR/oracle-nano-migrate.cast"
